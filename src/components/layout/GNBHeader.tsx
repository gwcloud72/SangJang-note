...
export function GNBHeader({ appName, source, tab, navItems, onTabChange, onRefresh, refreshing, liveText }: GNBHeaderProps) {
  ...
  return <header className="sticky top-0 z-40 flex h-topbar items-center border-b border-ink-200 bg-white px-ds-2 desktop:px-ds-3">
    ...
    <nav className="flex h-full min-w-0 flex-1 items-center gap-0" aria-label="상단 메뉴">
      {mainItems.map((item) => { const active = tab === item.id; return <button type="button" key={item.id} aria-current={active ? 'page' : undefined} onClick={() => onTabChange(item.id)} className={`gnb-tab flex h-full shrink-0 items-center border-b-3 px-2 text-[13px] transition focus:outline-none focus:ring-4 focus:ring-primary-100 desktop:px-3 desktop:text-[15px] ${active ? 'gnb-tab-active text-primary-600' : 'text-ink-700 hover:text-primary-600'}`}>{item.label}</button>; })}
      {moreItems.length ? <details className="relative h-full shrink-0">
        <summary className={`gnb-tab flex h-full cursor-pointer list-none items-center gap-1 border-b-3 px-2 text-[13px] transition marker:hidden focus:outline-none desktop:px-3 desktop:text-[15px] ${moreActive ? 'gnb-tab-active text-primary-600' : 'text-ink-700 hover:text-primary-600'}`}>더보기<ChevronDown size={14} strokeWidth={1.8} /></summary>
        <div className="absolute right-0 top-full z-50 min-w-28 rounded-md border border-ink-200 bg-white p-1 shadow-popover">
          {moreItems.map((item) => { const active = tab === item.id; return <button type="button" key={item.id} aria-current={active ? 'page' : undefined} onClick={() => onTabChange(item.id)} className={`block w-full rounded px-3 py-2 text-left text-sm ${active ? 'bg-primary-50 font-bold text-primary-600' : 'text-ink-700 hover:bg-ink-50'}`}>{item.label}</button>; })}
        </div>
      </details> : null}
    </nav>
